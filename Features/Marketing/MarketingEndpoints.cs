...
public static class MarketingEndpoints
{
    ...
    public static void MapMarketing(this WebApplication app)
    {
        ...
        group
            .MapPost(
                ...
                (CouponRequest r, MarketingService service, CancellationToken ct) =>
                    ApiResponse.OkAsync(service.CreateAsync(r, ct))
            )
        ...
        group
            ...
            .WithSummary("更新优惠券规则")
            .WithDescription(
                "仅 operator。id 是优惠券公开 UUID；已有核销记录后不能改变会影响历史业务的关键优惠规则，冲突返回 409。成功返回 200，data 为 null。"
            )
            .Produces<ApiResponse<object?>>(400)
            .Produces<ApiResponse<object?>>(401)
            .Produces<ApiResponse<object?>>(403)
        ...
        group
            ...
            .WithDescription(
                "仅 operator。按 isActive 更新启用标记；已过期优惠券不能启用，历史核销不删除。成功返回 200，data 为 null。"
            )
            .Produces<ApiResponse<object?>>(400)
            .Produces<ApiResponse<object?>>(401)
            .Produces<ApiResponse<object?>>(403)
