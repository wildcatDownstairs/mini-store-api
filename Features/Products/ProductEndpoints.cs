...
public static class ProductEndpoints
{
    ...
    public static void MapProducts(this WebApplication app)
    {
        ...
        store
            .MapGet(
                ...
                (string slug, ProductService service, CancellationToken ct) =>
                    service.GetBySlugAsync(slug, ct)
            )
        ...
        store
            .MapGet(
                "/brands",
                (ProductService service, CancellationToken ct) => service.BrandsAsync(ct)
            )
        ...
        store
            .MapGet(
                "/categories",
                (ProductService service, CancellationToken ct) => service.CategoriesAsync(ct)
            )
        ...
        admin
            .MapPost(
                ...
                async (ProductRequest request, ProductService service, CancellationToken ct) =>
                {
                    ...
                    return TypedResults.Created($"/api/admin/products/{result.Id}", result);
                }
            )
        ...
        admin
            ...
            .WithSummary("修改商品状态")
            .WithDescription(
                "仅 operator。接受 draft/active/inactive/archived 及当前 version；冲突返回 409。首次 active 时补发布时间，成功返回 204。"
            )
            .ProducesProblem(400)
            .ProducesProblem(401)
            .ProducesProblem(403)
