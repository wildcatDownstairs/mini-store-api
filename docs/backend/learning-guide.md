...
运行 API，访问 `/api/store/products?page=1&pageSize=8`。从 `Program.cs` 的 `app.MapCatalog()` 找到 `Features/Catalog/CatalogEndpoints.cs`。`MapGet` 把 URL 连接到处理函数；函数参数中的 `StoreDbContext db` 由依赖注入提供，不需要手动 new。
